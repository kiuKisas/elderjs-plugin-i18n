...
const fillDictionary = async (dictionary, allRequests) => {
  return allRequests.forEach((request) => {
...
const fillDictionaryWithCopy = async (dictionary, allRequests, getOtherData = async () => { return {} }) => {
  return allRequests.forEach(async (request) => {
    ...
    const dicoRoute = dictionary[request.locale][request.route] || {}
    const otherData = await getOtherData(request)
    dicoRoute[request.slug] = Object.assign({}, request, otherData)
    dictionary[request.locale][request.route] = dicoRoute
  })
}
...
const getPermalink = async (request, { routes, settings, helpers }) => {
  ...
  const permalink = await this.routes[request.route].permalink({
    request,
